...
async function parenthood(pgs, tree) {
	for (let i=0; i<pgs.length; i++) {
		...
		if (clist.length != chids.length) throw new Error(`${pg.props.title} does not have the right number of children!`);
		if (plist.length != pids.length) throw new Error(`${pg.props.title} does not have the right number of parents!`);
		for (let j=0; j<clist.length; j++) {
			if (!chids.includes(pgs[clist[j]].id)) throw new Error(`${pg.props.title} is missing child ${pgs[clist[j]].props.title}!`);
		}
		for (let j=0; j<plist.length; j++) {
			if (!pids.includes(pgs[plist[j]].id)) throw new Error(`${pg.props.title} is missing parent ${pgs[plist[j]].props.title}!`);
		}
	}
}
...
async function test() {
	...
	const author = await User.create("author", "authorfoobar", "author@compendium.com");
	const author2 = await User.create("author2", "author2foobar", "author2@compendium.com");
	if (!(await User.is(author))) throw new Error("Could not create author");
	if (!(await User.is(author2))) throw new Error("Could not create author2");
	await author.permission_level(user.PERM_ROOT);
	await author2.permission_level(user.PERM_ROOT);
	...
	for (let i=0; i<pgs.length; i++) {
		...
		for (let j=0; j < identifiers.length; j++) {
			if (!(await Page.is(pg.props[Page._identifiers[identifiers[j]]], identifiers[j]))) {
	...
	if ((await c1.parent()).id != book.id) throw new Error(`C1's parent set to ${c1.props.parent} (by get)`);
	if (c2.props.parent != book.id) throw new Error(`C2's parent set to ${c2.props.parent} (by props)`);
	if ((await c2.parent()).id != book.id) throw new Error(`C2's parent set to ${c2.props.parent} (by get)`);
	if (p1.props.parent != c1.id) throw new Error(`P1's parent set to ${p1.props.parent} (by props)`);
	if ((await p1.parent()).id != c1.id) throw new Error(`P1's parent set to ${p1.props.parent} (by get)`);
	if (p2.props.parent != c1.id) throw new Error(`P2's parent set to ${p2.props.parent} (by props)`);
	if ((await p2.parent()).id != c1.id) throw new Error(`P2's parent set to ${p2.props.parent} (by get)`);
	if (p3.props.parent != c1.id) throw new Error(`P3's parent set to ${p3.props.parent} (by props)`);
	if ((await p3.parent()).id != c1.id) throw new Error(`P3's parent set to ${p3.props.parent} (by get)`);
	if (pn.props.parent != c1.id) throw new Error(`PN's parent set to ${pn.props.parent} (by props)`);
	if ((await pn.parent()).id != c1.id) throw new Error(`PN's parent set to ${pn.props.parent} (by get)`);
	...
	console.log("Completed phase 1 of heirarchy tests...");

	const tree2 = [
		{parents: [], children: [1, 2]},
		{parents: [0], children: [3, 5, 6]},
		{parents: [0], children: [4]},
		{parents: [0, 1], children: []},
		{parents: [0, 2], children: []},
		{parents: [0, 1], children: []},
		{parents: [0, 1], children: []}
	];
	...
	const tree3 = [
		{parents: [], children: [1]},
		{parents: [0], children: [2, 3, 5]},
		{parents: [0, 1], children: [4]},
		{parents: [0, 1], children: []},
		{parents: [0, 1, 2], children: [6]},
		{parents: [0, 1], children: []},
		{parents: [0, 1, 2, 4], children: []}
	];

	await p2.parent(c2);
	await parenthood(pgs, tree2);

	console.log("Completed phase 2 of heirarchy tests...");

	await c2.parent(c1);
	await pn.parent(p2);
	await parenthood(pgs, tree3);
