...
export class Component extends Interactable(React.Component) {
	...
	get root() {
		return (this.props.root) ? this.props.root : this;
	}
	...
	add_children(children) {
		if (typeof children == 'string' || (children instanceof Array && children.length == 1 && typeof children[0] == 'string')){
			children = (typeof children == 'string') ? children : children[0];
			this.add_child(children);
		} else if (children instanceof Array) {
			for (let i = 0; i < children.length; i++) {
				let { type, props } = children[i];
				let grandchildren = children[i].children;
				this.add_child(type, props).add_children(grandchildren);
			}
		} else console.error("Attempted to add children with strange children structure");
	}
	...
	render() {
		...
		return e(this.render_src, {...this.attrs, className: this.state.class, ref: this.element.bind(this)}, this.state.children);
	}
}